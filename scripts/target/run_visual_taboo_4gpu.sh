...
ORGANISM_ID="${ORGANISM_ID:?Set ORGANISM_ID to a generated taboo-* organism id}"
EXTRA_ARGS=()
if [[ "${SMOKE:-0}" == "1" ]]; then
...
"$TORCHRUN" --nproc_per_node="$NPROC_PER_NODE" -m nl_probes.target_training \
  ...
  --train-jsonl data/train/visual_taboo/sft.jsonl \
  --eval-jsonl data/val/visual_taboo/sft.jsonl \
