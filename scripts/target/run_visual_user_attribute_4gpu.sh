...
ORGANISM_ID="${ORGANISM_ID:?Set ORGANISM_ID to a generated user-attribute-* organism id}"
EXTRA_ARGS=()
if [[ "${SMOKE:-0}" == "1" ]]; then
...
"$TORCHRUN" --nproc_per_node="$NPROC_PER_NODE" -m nl_probes.target_training \
  --organism visual_user_attribute \
  --organism-id "$ORGANISM_ID" \
  --train-jsonl data/train/visual_user_attribute/sft.jsonl \
  --eval-jsonl data/val/visual_user_attribute/sft.jsonl \
