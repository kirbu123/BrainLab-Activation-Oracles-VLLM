#!/usr/bin/env bash
# Download LLaVA-Instruct 150K annotations, COCO train2017 images,
# SNLI-VE / SNLI labels, and Flickr30k images for Visual AO training.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
DATA="${VLM_AO_DATA_DIR:-$ROOT/data}"
mkdir -p "$DATA/llava" "$DATA/coco" "$DATA/flickr30k" "$DATA/snli_ve"

echo "Data root: $DATA"
echo "See $DATA/README.md for licenses. Pixels keep upstream COCO/Flickr terms."
...
COCO_ZIP="$DATA/coco/train2017.zip"
...
if [[ ! -d "$COCO_DIR" ]] || [[ -z "$(ls -A "$COCO_DIR" 2>/dev/null || true)" ]]; then
  ...
  if [[ -s "$COCO_ZIP" ]] && [[ $(stat -c%s "$COCO_ZIP") -gt 100000000 ]]; then
    ...
    unzip -q -n "$COCO_ZIP" -d "$DATA/coco"
  else
...
SNLIVE_JSONL="$DATA/snli_ve/snli_ve_dev.jsonl"
...
FLICKR_DIR="$DATA/flickr30k/flickr30k-images"
FLICKR_TAR="$DATA/flickr30k/flickr30k_images.tar.gz"
if [[ ! -d "$FLICKR_DIR" ]] || [[ -z "$(ls -A "$FLICKR_DIR" 2>/dev/null || true)" ]]; then
  ...
  if [[ -s "$FLICKR_TAR" ]]; then
    ...
    tar -xzf "$FLICKR_TAR" -C "$DATA/flickr30k"
    if [[ -d "$DATA/flickr30k/flickr30k_images" && ! -d "$FLICKR_DIR" ]]; then
      ln -sfn "$DATA/flickr30k/flickr30k_images" "$FLICKR_DIR"
    fi
  fi
fi
...
echo "SNLI-VE json: $DATA/snli_ve/snli_ve_dev.jsonl $( [[ -f "$DATA/snli_ve/snli_ve_dev.jsonl" ]] && echo OK || echo MISSING )"
